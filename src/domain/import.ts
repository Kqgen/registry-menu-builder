...
function newTweak(value: ImportedRegistryValue): RegistryTweak {
  const group = value.keyPath.split("\\").at(-1) || "Registry";
  const source = value.sourceName.replace(/[\u0000-\u001f\u007f-\u009f]/gu, "").slice(0, 72);
  return {
    id: createTweakId(),
    label: (value.valueName || "(Default)").slice(0, 60),
    group: group.slice(0, 40),
    description: `${source} からインポート`.slice(0, 120),
    hive: value.hive,
